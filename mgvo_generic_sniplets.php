...
class MGVO_GEN_SNIPLET extends MGVO_SNIPLET {
	...
	//  Im MGVO Kalender werden, je nach Eintragsursprung eine Vielzahl unterschiedlicher Felder gefüllt, daher ist dies in manchen Fällen notwendig.
	 ...
	 * @return string
	 */
	function mgvo_gen_sniplet_gruppen(
	  ...
	  string|array $sort = NULL, string|array $filter = NULL,
	  $rewrite_fields = NULL, int $max_count = 0, int $page = 0): string {
		...
		return $this->mgvo_generic_sniplet($resar, "mgvo-vkal", $use_fields_table, $head_fields_table,
		  $sanitize_fields_table, $rewrite_fields, $sort, $filter, $max_count, $page);
	}
	 ...
	 * @return string
	 */
	function mgvo_gen_sniplet_vkal(int $vkalnr, int $seljahr,
	  array $vkal_use_fields_table = ["startdat", "startzeit", "bez", "ort"],
	  array $vkal_head_fields_table = ["Datum", "Start", "Veranstaltungen", "ort"],
	  string|array $vkal_sort = NULL, string|array $vkal_filter = NULL,
	  array $vkal_rewrite_fields = NULL, int $max_count = 0, int $page = 0): string {
		...
		return $this->mgvo_generic_sniplet($resar, "mgvo-vkal", $vkal_use_fields_table, $vkal_head_fields_table,
		  $vkal_sanitize_fields_table, $vkal_rewrite_fields, $vkal_sort, $vkal_filter, $max_count, $page);
	}
	 ...
	 * @return string
	 */
	function mgvo_generic_sniplet(
	  array $resar, string $css_class, array $use_fields_table, array $head_fields_table, array $sanitize_fields_table,
	  array $rewrite, string|array|null $sort, string|array $filter, int $max_count, int $page): string {
		...
		// Filter können entweder als String mit "feld=Wert,feld=wert,...' übergeben werden (mit Komma getrennt) oder als Array.
		...
		foreach ($listarray as $idx => $vkr) {
			...
			if (strstr($vkr['bez'], "EXTERN:")) {
				$vkr['bez'] = str_replace("EXTERN:", "",
				  strstr($vkr['bez'], "EXTERN:")); // Titel ab Extern: (ohne Extern:)
			}
	...
	function mgvo_gen_sniplet_vkal_entry(int $vkalnr, int $seljahr, $arrayindex, $use_fields_table = NULL,
	  $head_fields_table = NULL, $vkal_rewrite_fields = NULL): string {
		...
		return $this->mgvo_generic_sniplet_entry($resar, $arrayindex, "mgvo-vkal-entry", $use_fields_table,
		  $head_fields_table, $sanitize_fields_table, $vkal_rewrite_fields);
	}

	function mgvo_gen_sniplet_gruppen_entry($arrayindex, $use_fields_table = NULL, $head_fields_table = NULL): string {
		// Liest den Vereinskalender mit Nr. vkalnr mit Terminen des Jahres seljahr und gibt den Temrin mit der EventID aus.
		//  Verfügbare Felder: startdat,bez,prio,vkalnr,evnr,startzeit,enddat,endzeit,ortid,notiz,rec_day_freq,rec_wk_freq,rec_mon_freq1,rec_mon_tag,rec_mon_freq2,rec_yr_freq1,rec_yr_tag,rec_yr_freq2,rec_range_enddat,ort,ortkb
		// Konfig-Anleitung siehe oben
		if (empty($use_fields_table) or empty($head_fields_table)) {
		...
		return $this->mgvo_generic_sniplet_entry($resar, $arrayindex, "mgvo-gruppen-entry", $use_fields_table,
		  $head_fields_table, $sanitize_fields_table, NULL);
	}

	function mgvo_gen_sniplet_event_entry($arrayindex, $use_fields_table = NULL, $head_fields_table = NULL): string {
		...
		return $this->mgvo_generic_sniplet_entry($resar, $arrayindex, "mgvo-event-entry", $use_fields_table,
		  $head_fields_table, $sanitize_fields_table, NULL);
	}

	function mgvo_gen_sniplet_notraining_entry($arrayindex, $use_fields_table = NULL,
	  $head_fields_table = NULL): string {
		// Liest den Veranstaltungseintrag mit Nr.
		...
		return $this->mgvo_generic_sniplet_entry($resar, $arrayindex, "mgvo-notraining-entry", $use_fields_table,
		  $head_fields_table, $sanitize_fields_table, NULL);
	}


	function mgvo_generic_sniplet_entry($resar, $arrayindex, $css_class, $use_fields_table, $head_fields_table,
	  $sanitize_fields_table, $rewrite): string {
